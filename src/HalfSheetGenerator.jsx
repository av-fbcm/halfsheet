...
const LOGO_URL = "https://static.wixstatic.com/media/81f7ac_11ee3bfca47c4a11b3afcf00bf18e90a~mv2.png";
...
const GOLD = "#b5923a";
const DARK = "#1a1a2e";
const CLIENT_ID = "700317661922-ainbohgsk55n368agtauob97o2asc9ig.apps.googleusercontent.com";
...
function getNextSunday(dateStr) {
  ...
  try {
    // Strip leading day name if present e.g. "Wednesday, March 12, 2025" → "March 12, 2025"
...
function Logo({ failed, setFailed }) {
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", marginBottom: "6px" }}>
      <img src={TOWER_LOGO} alt="" style={{ height: "40px", width: "auto", objectFit: "contain", flexShrink: 0 }} />
      {failed ? (
        <div style={{
          fontFamily: "'Georgia', serif", fontWeight: "bold", fontSize: "13px",
          color: DARK, letterSpacing: "0.04em", lineHeight: 1.2
        }}>
          FIRST BAPTIST<br />CHURCH MUNCIE
        </div>
      ) : (
        <img src={LOGO_URL} alt="FBC Muncie"
...
function AnnouncementItem({ item, isLast }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: "10.5px", fontWeight: "bold", color: DARK, lineHeight: 1.25, marginBottom: "1.5px" }}>
        {item.title}
        {item.date && (
          <span style={{ fontWeight: "normal", color: GOLD, marginLeft: "5px", fontSize: "9.5px" }}>
            {item.date}
          </span>
        )}
      </div>
      {item.description && <div style={{ fontSize: "9px", color: "#333", lineHeight: 1.45 }}>{item.description}</div>}
      {item.location && <div style={{ fontSize: "8px", color: "#666", marginTop: "2px" }}>📍 {item.location}</div>}
      {item.registration && <div style={{ fontSize: "8px", color: "#555", marginTop: "1.5px" }}>→ {item.registration}</div>}
    </div>
  );
}
...
// ─── Front half-sheet ─────────────────────────────────────────────────────────
function HalfSheetFront({ data, logoFailed, setLogoFailed }) {
  const outerRef = useRef(null);
  const innerRef = useRef(null);
  useAutoShrink(outerRef, innerRef);
  const front = (data?.announcements || []).slice(0, FRONT_MAX);
  const hasBack = (data?.announcements || []).length > FRONT_MAX;
  return (
    <div ref={outerRef} style={{
      ...
      <div ref={innerRef} style={{
        ...
        <div>
          {front.map((item, i) => (
            <AnnouncementItem key={i} item={item} isLast={i === front.length - 1} />
          ))}
        </div>
        <div style={{ flex: 1 }} />
        ...
        <div style={{ borderTop: "0.5px solid #ddd", paddingTop: "5px", display: "flex", justifyContent: "space-between" }}>
          ...
          {hasBack && <div style={{ fontSize: "7px", color: GOLD, fontFamily: "Arial, sans-serif", fontStyle: "italic" }}>continued on back</div>}
  ...
  function buildTwoColHTML(d, wordMode, ri, bd) {
    ...
    const annoItem = (item, isLast) => `
      <div style="margin-bottom:5pt;padding-bottom:5pt;${isLast ? "" : "border-bottom:0.5pt solid #eee;"}">
        <div style="font-size:10pt;font-weight:bold;color:#1a1a2e;line-height:1.25;">
          ${item.title}${item.date ? `<span style="font-weight:normal;color:#b5923a;font-size:9pt;"> &nbsp;${item.date}</span>` : ""}
        </div>
        ${item.description ? `<div style="font-size:8.5pt;color:#333;line-height:1.4;">${item.description}</div>` : ""}
        ${item.location   ? `<div style="font-size:7.5pt;color:#666;margin-top:1.5pt;">&#128205; ${item.location}</div>` : ""}
        ${item.registration ? `<div style="font-size:7.5pt;color:#555;margin-top:1.5pt;">&#8594; ${item.registration}</div>` : ""}
      </div>`;

    const sectionHead = (label) =>
      `<div style="font-size:8pt;letter-spacing:0.12em;text-transform:uppercase;color:#b5923a;font-family:Arial,sans-serif;font-weight:bold;border-bottom:0.5pt solid #ddd;padding-bottom:3pt;margin-bottom:7pt;">${label}</div>`;

    const noteHead = (label) =>
      `<div style="font-size:7.5pt;letter-spacing:0.12em;text-transform:uppercase;color:#b5923a;font-family:Arial,sans-serif;font-weight:bold;margin-top:6pt;margin-bottom:2pt;">${label}</div>`;
    ...
    const sermonBlock = d.sermon ? `
      <table cellpadding="0" cellspacing="0" border="0" style="width:100%;margin-bottom:7pt;background-color:#fdf8f0;border:1pt solid #b5923a;border-left:4pt solid #b5923a;"><tr>
        <td style="padding:6pt 8pt;background-color:#fdf8f0;">
          ${d.sermon.series ? `<div style="font-size:7.5pt;letter-spacing:0.12em;text-transform:uppercase;color:#b5923a;font-family:Arial,sans-serif;font-weight:bold;margin-bottom:2pt;">${d.sermon.series}</div>` : ""}
          ...
          ${d.sermon.scripture ? `<div style="font-size:9pt;color:#555;font-style:italic;${d.sermon.teaser ? "margin-bottom:3pt;" : ""}">${d.sermon.scripture}</div>` : ""}
          ${d.sermon.teaser ? `<div style="font-size:8.5pt;color:#444;line-height:1.4;">${d.sermon.teaser}</div>` : ""}
        </td>
      </tr></table>` : "";
    ...
    const connectFooter = `
      <div style="border-top:1.5pt solid #b5923a;padding-top:5pt;margin-top:5pt;">
        <table cellpadding="0" cellspacing="0" border="0" style="width:100%;margin-bottom:4pt;"><tr>
          ...
          <td style="width:56pt;text-align:right;vertical-align:middle;">
            <img src="${QR_CODE}" style="height:52pt;width:52pt;" alt="QR"/>
          </td>
        </tr></table>
    ...
    const miniFooter = `
      <table cellpadding="0" cellspacing="4pt" border="0" style="width:100%;border-top:0.5pt solid #ddd;margin-top:8pt;"><tr>
        <td style="font-size:7pt;color:#999;font-family:Arial,sans-serif;">fbcmuncie.org</td>
        ${hasBack ? `<td style="font-size:7pt;color:#b5923a;font-family:Arial,sans-serif;font-style:italic;text-align:center;">continued on back</td>` : `<td></td>`}
        <td style="font-size:7pt;color:#999;font-family:Arial,sans-serif;text-align:right;">Church Center App</td>
      </tr></table>`;
...
${bodyWrap(pageTable(frontCell) + pageTable(backCell))}
  ...
  function downloadWord() {
    ...
    try {
      ...
      a.href = url;
      a.download = "FBC-HalfSheet-" + (data.date || "weekly").replace(/[^a-zA-Z0-9]/g, "-") + ".doc";
  ...
  function buildPrintHTML(d, ri, bd) {
    ...
    const annoItem = (item, isLast) => `
      <div style="margin-bottom:7px;padding-bottom:7px;${isLast ? "" : "border-bottom:0.5px solid #eee;"}">
        <div style="font-size:10.5px;font-weight:bold;color:#1a1a2e;line-height:1.25;margin-bottom:1.5px;">
          ${item.title}${item.date ? `<span style="font-weight:normal;color:#b5923a;margin-left:5px;font-size:9.5px;">${item.date}</span>` : ""}
        </div>
        ${item.description ? `<div style="font-size:9px;color:#333;line-height:1.45;">${item.description}</div>` : ""}
        ${item.location ? `<div style="font-size:8px;color:#666;margin-top:2px;">&#128205; ${item.location}</div>` : ""}
        ${item.registration ? `<div style="font-size:8px;color:#555;margin-top:1.5px;">&#8594; ${item.registration}</div>` : ""}
      </div>`;
    ...
    const sermonHtml = d.sermon ? `
      <div style="background:#fdf8f0;border:1px solid #b5923a;border-left:3.5px solid #b5923a;border-radius:3px;padding:7px 9px;margin-bottom:10px;">
        ${d.sermon.series ? `<div style="font-size:7.5px;letter-spacing:0.14em;text-transform:uppercase;color:#b5923a;font-family:Arial,sans-serif;font-weight:bold;margin-bottom:2px;">${d.sermon.series}</div>` : ""}
        ...
        ${d.sermon.scripture ? `<div style="font-size:9px;color:#555;font-style:italic;${d.sermon.teaser ? "margin-bottom:3px;" : ""}">${d.sermon.scripture}</div>` : ""}
        ${d.sermon.teaser ? `<div style="font-size:8.5px;color:#444;line-height:1.4;">${d.sermon.teaser}</div>` : ""}
      </div>` : "";
    ...
    const connectFooterHtml = `
      <div style="border-top:1.5px solid #b5923a;padding-top:8px;margin-top:4px;">
      ...
      <div style="${hs}">
        <div style="${inner}">
          ...
          <div style="border-top:0.5px solid #ddd;padding-top:5px;display:flex;justify-content:space-between;">
            ...
            ${hasBack ? `<div style="font-size:7px;color:#b5923a;font-family:Arial,sans-serif;font-style:italic;">continued on back</div>` : ""}
...
Rules: include up to 8 most important announcements. Sermon block may be null. Keep descriptions under 25 words.`,
  ...
  const sharedLogoProps = { logoFailed, setLogoFailed };
                    ...
                    <div key={i} className="ann-card">
                      <div className="ann-card-header">
                        <span className="ann-card-label">
                          #{i + 1}
                          <span>{i < FRONT_MAX ? "Front" : "Back"}</span>
                        </span>
                ...
                <div className="preview-row">
                  ...
                  <div className="preview-scaled">
                    <div className="preview-wrap">
                      <HalfSheetFront data={data} {...sharedLogoProps} />
                      <div className="cut" />
                      <HalfSheetFront data={data} {...sharedLogoProps} />
                    </div>
                  </div>
